...
from bot.user_state import UserStates, MAX_OPUS_CALLS
...
_HANDOFF_ES = ("{name}: esto quizá necesite ayuda más directa. Avisé a "
               "[el instructor](tg://user?id={host_id}) para que lo revise cuando pueda.")
_SUMMARY_LEN = 200
...
class Handler:
    def __init__(self, cfg: Config, client: ClaudeClient, memory: RecentQA,
                 ...
                 user_states: UserStates | None = None):
        self.cfg = cfg
        self.client = client
        self.memory = memory
        ...
        self.sender = sender
        self.now_fn = now_fn
        self.user_states = user_states or UserStates()
    ...
    async def _deliver_safe(self, msg: IncomingMessage, source_message_id: int, text: str) -> None:
        try:
            await self._deliver(msg, source_message_id, text)
        except Exception:
    ...
    async def handle(self, msg: IncomingMessage, source_message_id: int,
        ...
        if not should_respond(msg, self.cfg).respond:
            return
        now = self.now_fn()
        if not self.rate_limiter.allow(msg.user_id, now):
            return
        state = self.user_states.get(msg.user_id, now)
        history_text = state.render_history()          # this user's PRIOR messages
        try:
            answer = await self.client.answer(msg.text, images, self.memory, history_text)
        except Exception:
            log.exception("Answer generation failed")
            await self._safe_send(_ERROR_ES.format(name=msg.first_name), source_message_id)
            return
        state.messages.append(msg.text)                # record this attempt

        if not (answer.escalate and self.cfg.escalation_enabled):
            await self._deliver_safe(msg, source_message_id, answer.text)
            return

        if state.opus_calls < MAX_OPUS_CALLS:
            # Tier 2: escalate to Opus in the background.
            state.opus_calls += 1
